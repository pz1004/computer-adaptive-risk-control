...
This guide takes you from the current artifacts (`draft.md` + the root `carc/`
reference implementation) to a submittable TMLR paper, using Codex in this repository.
...
**Original acceptance gap:** the draft §10.2 real-data study was planned but not run. The first
Codex completion pass closed the minimum CIFAR gap by adding a real branchy-CNN cache diagnostic
(`results/cifar_branchy_real_cache_eval*.json`). Remaining ImageNet, language, tabular, and real
shift studies still strengthen the claim-evidence match. Everything below exists to keep that
work sequenced **without disturbing the proven core**.
...
**do not chase numbers** — the draft is already written to accept that outcome. Reducing a
claim is a valid TMLR remedy; fabricating a stronger one is not.
...
## What this repo is
Reference code + experiments for a TMLR paper on distribution-free risk control for
early-exit networks. Theory is frozen; we are running the real-data study (`draft.md` §10.2).
...
This is the minimum experiment that lets the paper pass Criterion 1. Do it carefully and in
...
## 4. Phase 3 — Scale out (optional but strengthens interest)
...
3. **LLM size-cascade** — small→medium→large as a 3-exit policy on a text-classification set.
   ...
   most reviewer-interesting, but also the most engineering. Treat as a stretch goal.
...
## 7. Phase 6 — LaTeX, citations, TMLR formatting

- Convert `draft.md` to the **TMLR LaTeX template** (download from the TMLR site). Ask
...
- TMLR specifics: review is double blind and submissions must be anonymized; there is no
  hard page limit, but unusually long papers may slow review. Use the official TMLR LaTeX
  style/template, keep supplementary material anonymized, include a reproducibility statement,
  and add an anonymized code/supplement link when submitting. Confirm current requirements on
  the TMLR site before submitting.
...
Re-apply the two TMLR criteria from the last audit, now against the finished paper:

**Criterion 1 (claims ↔ evidence).** Walk every claim sentence in the abstract, contributions,
...
**Criterion 2 (interest).** Confirm the practical takeaway leads: *naive confidence-threshold
tuning silently violates target risk; a distribution-free certificate fixes it at near-naive cost*,
now demonstrated on at least one real model.
...
- **Make Codex argue against you.** After it backfills a result, ask: "as a skeptical TMLR
...
5. Phases 3–4 — more datasets, shift, joint, ablations — strengthen interest and robustness.
